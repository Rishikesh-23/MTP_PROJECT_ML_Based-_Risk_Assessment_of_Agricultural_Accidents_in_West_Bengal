...
# AIS severity details with fatal/non-fatal category
ais_info = {
...
severity, description, color, accident_category = ais_info[prediction]

# Displaying Results with Fatal/Non-Fatal clearly indicated
st.markdown(f"""
<div style="background-color:{color};padding:15px;border-radius:10px;text-align:center">
    <h2 style="color:white;">AIS Level: {prediction}</h2>
    <h3 style="color:white;">Classification: {severity}</h3>
    <h3 style="color:white;">Accident Category: {accident_category}</h3>
...
st.markdown("---")
st.subheader("📝 AIS Severity Scale (with Accident Category)")
ais_df = pd.DataFrame({
    ...
    "Accident Category": ["Non-Fatal", "Non-Fatal", "Non-Fatal", "Non-Fatal", "Fatal", "Fatal"]
})
